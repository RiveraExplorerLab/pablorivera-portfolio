...
import { useProjects } from '../lib/useSupabase' // ⬅️ use Supabase

export default function Home() {
  const { data: projects } = useProjects()
  // Always feature Explorer Lab; fall back to first project if missing
  const featured =
    projects?.find(
      (p: any) =>
        (p.slug && p.slug.toLowerCase() === 'explorer-lab') ||
        (p.name && p.name.toLowerCase().includes('explorer lab'))
    ) || projects?.[0]
              ...
              <p className="text-stone-300 text-sm mt-1">
                ...
                {featured.details_md && (
                  <span className="block mt-1 text-stone-400">
                    {/* Optional teaser from details_md */}
                    {featured.details_md.slice(0, 120)}…
                  </span>
                )}
              </p>
              <div className="mt-3 flex gap-4 text-sm">
                {/* Removed “See the Story” (no /projects/:slug route) */}
                <Link
...
/** Card component with subtle emerald accent dot */
